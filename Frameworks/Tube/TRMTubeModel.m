...
- (void)initializeConversion:(TRMInputParameters *)inputParameters;
{
    ...
    [self initializeFilter:&m_sampleRateConverter];
    ...
    roundedSampleRateRatio = pow(2.0, FRACTION_BITS) / (double)m_sampleRateConverter.timeRegisterIncrement;
    ...
    // Calculate pad size
    padSize = (m_sampleRateConverter.sampleRateRatio >= 1.0) ? ZERO_CROSSINGS :
...
- (void)initializeFilter:(TRMSampleRateConverter *)sampleRateConverter; // TODO (2012-05-19): This doesn't need the parameter now.
{
    double x, IBeta;
    int32_t i;
