...
{
//	printf("Displaying Special Event\n");
	[self clearView];
	[self drawGrid];
	[self drawEquations];
	[self drawPhones];
...
NSRect rect = NSMakeRect(0, 0, 10, 10);
	...
	for(i = 0; i< [[currentTemplate points] count]; i++)
	{
		...
		if ([currentPoint expression] == nil)
			...
			time = (float) [[currentPoint expression] 
					 evaluate: symbols 
					 phones: dummyPhoneList 
					 andCacheWith: cache];

//		printf("%x Time = %f\n", [currentPoint expression], time);
...
//		printf("y = %f\n", y);
		if ([currentPoint expression] == nil)
			...
			eventTime = [[currentPoint expression] 
				      evaluate: symbols 
				      phones: dummyPhoneList 
				      andCacheWith: cache];
